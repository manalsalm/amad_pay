...
def forecast_all_categories(csv_file, periods=7):
    ...
    for category, forecast_df in all_forecasts.items():
        ...
        for _, row in forecast_df.iterrows():
            ...
            print(f"  Explanation: You are expected to spend around ${predicted:.2f} on {category} on {date}.")
...
def check_saving_target(csv_file, monthly_income, saving_target, forecast_period=30):
    ...
    if predicted_spending > budget_after_savings:
        ...
        return (f"Warning: Your predicted spending (${predicted_spending:.2f}) exceeds your "
                f"budget after savings (${budget_after_savings:.2f}) by ${excess:.2f}. "
